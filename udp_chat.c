...
int main(int argc, char **argv) {
    int port, listener, epoll_fd, accept_type;
    pthread_t stid, btid, rtid, intid, httid; // 发送， 心跳， 接收， 初始化
    ...
    pthread_create(&rtid, NULL, receiver_handler, &recv_fd);
    pthread_create(&httid, NULL, ht_handler, me);
    pthread_create(&intid, NULL, init_handler, &listener);
    while (1) {
        fprintf(stderr,  GREEN "epoll fd ok \n" NOCOLOR);
        ...
        for (int i = 0; i < nfds; i += 1) {
            ...
            if ((accept_type = udp_accept(listener, new_user)) < 0) {
                fprintf(stderr, RED "udp_accept error\n");
                free(new_user);
                continue;
            }
            ...
            if (check_node == NULL) {
                ...
                add_to_reactor(recv_fd, new_user);
